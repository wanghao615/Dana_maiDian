...
static void *DAAllocBufferForObjCType(const char *objCType)
{
    void *buffer = NULL;
    ...
    if (result != 0) {
        DAError(@"Error allocating aligned memory: %s", strerror(result));
    }
...
@implementation NSInvocation (DAHelpers)
...
- (void)sa_setArgument:(id)argumentValue atIndex:(NSUInteger)index
{
    ...
    else if ([argumentValue isKindOfClass:[NSValue class]])
    {
        ...
        void *buffer = DAAllocBufferForObjCType([valueArgument objCType]);
...
- (id)sa_returnValue
{
    ...
    void *buffer = DAAllocBufferForObjCType(objCType);
