...
@interface DAObjectFilter : NSObject
...
@implementation DAObjectSelector

+ (DAObjectSelector *)objectSelectorWithString:(NSString *)string {
    return [[DAObjectSelector alloc] initWithString:string];
}
...
- (NSArray *)selectFromRoot:(id)root evaluatingFinalPredicate:(BOOL)finalPredicate {
    ...
    if (root) {
        ...
        for (NSUInteger i = 0, n = [_filters count]; i < n; i++) {
            DAObjectFilter *filter = _filters[i];
...
- (Class)selectedClass {
    if ([_filters count] > 0) {
        return NSClassFromString(((DAObjectFilter *)_filters[[_filters count] - 1]).name);
    }
    return nil;
}
