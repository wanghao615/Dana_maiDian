...
@implementation UIView (DAHelpers)
...
// Set by a userDefinedRuntimeAttr in the DATagNibs.rb script
- (void)setDanaAnalyticsViewId:(id)object {
    objc_setAssociatedObject(self, @selector(danaAnalyticsViewId), [object copy], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
static NSString* sa_encryptHelper(id input) {
    NSString *DALT = @"dbba253e672cc94bee5da560040b47b1";
    NSMutableString *encryptedStuff = nil;
    if ([input isKindOfClass:[NSString class]]) {
        NSData *data = [[input stringByAppendingString:DALT]  dataUsingEncoding:NSUTF8StringEncoding];
