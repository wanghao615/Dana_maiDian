...
@implementation DAUITableViewBinding
...
+ (DAEventBinding *)bindingWithJSONObject:(NSDictionary *)object {
    ...
    if (triggerId <= 0) {
        DAError(@"binding requires a trigger id");
    }
    ...
    if (!tableDelegate) {
        DAError(@"binding requires a table_delegate class, path='%@', trigger_id='%ld', event_name='%@', delegate='%@'",
                path, triggerId, eventName, tableDelegateName ? tableDelegateName : @"Unset");
        return nil;
    }

    return [[DAUITableViewBinding alloc] initWithEventName:eventName
...
- (void)stop {
    if (self.running && self.swizzleClass != nil) {
        [DASwizzler unswizzleSelector:@selector(tableView:didSelectRowAtIndexPath:)
                              onClass:self.swizzleClass
                                named:self.name];
        self.running = false;
    }
